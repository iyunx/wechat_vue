...
const search = reactive<Tsearch>({
      ...
      userArr = reactive<{lists: {[k: string]: any}, star: any[], all: any[]}>({
        lists: {},
        star: [],
        all: [],
      }),
...
const searchBtn = () => {
  if(action == 'remove'){
    ...
    if(search.value.length){
      ...
      users.lists = {}
      for(let [k, v] of u) {
        const vm = v.filter((item: any) => item.name.includes(search.value) || item.Contact.rname.includes(search.value))
