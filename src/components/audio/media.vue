...
import vTrackDirective from './directives/track'
  ...
  watch(() => props.audioXY, ({pageX, pageY}: {pageX: number, pageY: number}) => {
    const perX = GetPercent(pageX, body.x)
    const perY = GetPercent(pageY, body.y)
    if(perX <= 50 && perY < 87) {
  ...
  const audioStart = () => {
    if(stream.value){
      try {
        ...
        recorder.value.ondataavailable = (e: {data: Blob}) => chunks.value.push(e.data)
      } catch (error) {
        console.log(error)
      }
    }
